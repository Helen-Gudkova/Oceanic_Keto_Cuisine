...
class RegisterUser(CreateView):
    ...
    success_url = reverse_lazy(
        'users:login')  # URL, на который будет перенаправлен пользователь после успешной регистрации
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                return redirect(reverse_lazy('recipes'))  # Перенаправляем на каталог, если next_url пуст
            else:
...
class UserCardsView(ListView):
    ...
    def get_queryset(self):
        return Recipe.objects.filter(author=self.request.user).order_by('-upload_date')
